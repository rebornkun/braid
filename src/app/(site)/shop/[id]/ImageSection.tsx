...
const ImageSection = ({
  ...
  return (
    <div className="flex-[1_1_50%] w-full lg:w-[50%] flex flex-col gap-8 justify-center">
      {isDataLoading ? (
        <div className="skeleton w-full h-[400px] lg:h-[500px] mx-auto transition-all ease-in-out duration-300 group-hover/collection:scale-[1.4] origin-bottom"></div>
      ) : (
        <Image
          src={data.image ? data.image : "/hairs/hairOne.webp"}
          alt="product"
          height={10000}
          width={10000}
          priority
            ...
            <div className="flex gap-4 w-full overflow-x-auto">
              {productsMock.slice(0, 4).map((datum: any, index: number) => {
                return (
                  <div
                    key={index}
                    className="flex-1 bg-white h-[200px] min-[300px] rounded-[5px] group/swiperImg bg-[#FFFFFF7B] flex flex-col gap-2 items-center justify-center shadow cursor-pointer "
                  >
                    <Image
                      src={datum?.image}
                      alt="hair"
